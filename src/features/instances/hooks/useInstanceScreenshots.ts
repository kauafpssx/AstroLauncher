...
export function useInstanceScreenshots(instanceId: string) {
  ...
  const load = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const list = await InstanceWorkspaceAPI.listScreenshots(instanceId)
      const loaded = await Promise.all(
        list.map(async (info) => ({ info, dataUri: await InstanceWorkspaceAPI.readScreenshot(instanceId, info.name) })),
      )
  ...
  return { shots, isLoading, reload: load, setShots }
}
