...
export function LogTab({ instanceId }: LogTabProps) {
  ...
  useEffect(() => {
    ...
    InstanceWorkspaceAPI.readLog(instanceId)
      .then((log) => {
        if (cancelled) return
        setContent(log || 'Nenhum log disponível ainda. Inicie a instância para gerar um.')
      })
      .catch((err) => {
        if (cancelled) return
  ...
  const handleReload = () => {
    setIsLoading(true)
    InstanceWorkspaceAPI.readLog(instanceId)
      .then((log) => setContent(log || 'Nenhum log disponível ainda. Inicie a instância para gerar um.'))
    ...
    <div className="flex h-full flex-col gap-3 overflow-hidden">
      ...
      <div className="relative min-h-0 flex-1">
        <pre
          ...
          className={cn(
            'h-full overflow-auto rounded-lg border bg-muted/30 p-3 font-mono text-xs leading-relaxed',
        ...
        {showScrollBtn && (
          <Button
            size="icon"
            variant="secondary"
            className="absolute bottom-3 right-3 size-7 rounded-full shadow"
            onClick={() => {
              scrollToBottom()
              setAutoScroll(true)
