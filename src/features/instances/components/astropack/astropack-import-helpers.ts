...
import type {
  AstroPackManifest,
  ExportSelection,
} from '@/features/instances/services/astropack.api'
...
export function computeImportEntries(
  ...
  return {
    ...
    total:
      ...
      (selection.configs ? manifest.configs.length : 0),
  }
}
export function buildImportCategoryItems(
  ...
  return manifest
    ? [
        ...
        {
          key: 'configs',
          label: 'Configs dos mods (pasta config)',
          count: manifest.configs.length,
