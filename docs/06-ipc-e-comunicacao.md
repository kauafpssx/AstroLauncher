...
| `instance_workspace_commands.rs` | 25 comandos: log, notas, arquivos de config, mundos, servers, screenshots, atalhos — ex.: `read_instance_log`, `list_instance_notes`, `write_instance_note`, `list_instance_config_files`, `write_instance_config_file`, `list_instance_worlds`, `delete_instance_world`, `list_instance_servers`, `add_instance_server`, `list_instance_screenshots`, `save_instance_screenshot_as`, `list_instance_shortcuts`, `toggle_instance_shortcut`, `refresh_instance_shortcut_icon` || `minecraft_commands.rs` | `list_minecraft_versions` (async), `launch_instance` (async), `stop_instance`, `cancel_launch`, `get_total_system_memory_mb`, `list_audio_output_devices`, `take_pending_launch` |
