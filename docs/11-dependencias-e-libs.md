...
| `flate2`           | 1.1.9              | Descompressão gzip de `level.dat` (seed dos mundos, v1.0.0)                    |
| `fastnbt`          | 2.6.3              | Parse NBT (`level.dat`, `servers.dat`) (v1.0.0)                                |
...
- **Modrinth API (v2)** — pública, sem key (base em `plugins.env` → `api.modrinth`)
...
| `rayon`       | 1.12.0 | Paralelismo data-parallel — tiles do Seed Map calculados em threads (v1.0.0)                                                                  |

`crossbeam`, `itertools`, `regex`, `semver` e `async-trait` foram **removidos** na v0.5.2 (sem uso).

### Build e Worldgen (Cubiomes vendorizado, v1.0.0)

| Biblioteca | Versão | Finalidade                                                                             |
| ---------- | ------ | -------------------------------------------------------------------------------------- |
| `cc`       | 1.4.0  | Build-dependency: compila o C vendorizado (`src-tauri/vendor/cubiomes/`) no `build.rs` |

O Seed Map usa o [Cubiomes](https://github.com/Cubitect/cubiomes) (C, MIT) vendorizado do fork https://github.com/xpple/cubiomes (commit `0a3db3a`, 2026-09-03, ver `VENDORED_COMMIT.txt`) — integração via FFI manual em `infrastructure/worldgen/`, sem os crates `cubiomes`/`cubiomes-sys` (bindings incompatíveis neste toolchain Windows). O `build.rs` compila só o subconjunto necessário com `clang-cl` no Windows (MSVC não aceita o C99 do fork); `vendor/update-cubiomes.sh` re-vendoriza do upstream preservando `rust_shim.c`.
...
| `wiremock` | 0.6.5  | Mock HTTP em testes de infra (desde v0.6.0)  |
...
### Seed Map (mapa, v1.0.0)

| Pacote | Versão  | Finalidade                                              |
| ------ | ------- | ------------------------------------------------------- |
| `ol`   | 10.10.0 | OpenLayers — mapa de biomas/estruturas (BSD-2-Clause)   |
