...
`src-tauri/app/infrastructure/`: `minecraft/` (manifest, rules, servers.dat, version meta), `java/` (detect, download, manager), `downloader/` (file + asset downloader, progress — sem retry apesar de `reqwest-retry` estar no Cargo.toml), `process/` (manager + launcher), `filesystem/` (paths), `discord/` (RPC), `modloader/` (fabric_like, forge_like, liteloader, profile), `curseforge/`, `modrinth/`, `playermc/` (skins), `persistence/` (sqlite, migrations, repositories, config). Forge/NeoForge delegam boa parte do trabalho para a crate externa `mc-launcher-core`.
