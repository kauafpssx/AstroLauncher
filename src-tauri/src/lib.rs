...
use presentation::commands::{
  account_commands, astropack_commands, custom_icon_commands, discord_commands, folder_commands, instance_commands,
  instance_workspace_commands, minecraft_commands, mod_commands, playtime_commands, settings_commands, skin_commands,
  splash_commands,
};
use tauri::Manager;
...
pub fn run() {
  tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      let discord_config = app.config().plugins.0.get("discord").cloned().unwrap_or_default();
      ...
      let discord_logo_asset_key = discord_config
        ...
        .to_string();
      let state = bootstrap::setup::build_app_state(&app_data_dir, discord_client_id, discord_logo_asset_key);
      app.manage(state);
